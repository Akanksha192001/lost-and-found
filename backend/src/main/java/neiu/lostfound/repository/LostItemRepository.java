...
public interface LostItemRepository extends JpaRepository<LostItem, Long> {
    ...
    List<LostItem> findByOwnerAddressContainingIgnoreCase(String ownerAddress);

    @Query("SELECT l FROM LostItem l WHERE (:q IS NULL OR LOWER(l.title) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.location) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.ownerName) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.ownerEmail) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.ownerAddress) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.description) LIKE LOWER(CONCAT('%',:q,'%')))")
